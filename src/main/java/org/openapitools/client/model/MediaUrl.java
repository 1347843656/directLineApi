...
public class MediaUrl {
  ...
  * @throws IOException if the JSON Object is invalid with respect to MediaUrl
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (MediaUrl.openapiRequiredFields.isEmpty()) {
          return;
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!MediaUrl.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MediaUrl` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
