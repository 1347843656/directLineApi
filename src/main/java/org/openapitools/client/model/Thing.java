...
public class Thing {
  ...
  * @throws IOException if the JSON Object is invalid with respect to Thing
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (Thing.openapiRequiredFields.isEmpty()) {
          return;
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Thing.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Thing` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
