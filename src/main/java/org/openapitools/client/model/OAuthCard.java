...
public class OAuthCard {
  ...
  * @throws IOException if the JSON Object is invalid with respect to OAuthCard
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (OAuthCard.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in OAuthCard is not found in the empty JSON string", OAuthCard.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!OAuthCard.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `OAuthCard` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!OAuthCard.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'OAuthCard' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<OAuthCard> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(OAuthCard.class));

       return (TypeAdapter<T>) new TypeAdapter<OAuthCard>() {
           @Override
           public void write(JsonWriter out, OAuthCard value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public OAuthCard read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of OAuthCard given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of OAuthCard
  * @throws IOException if the JSON string is invalid with respect to OAuthCard
  */
  public static OAuthCard fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, OAuthCard.class);
  }

 /**
  * Convert an instance of OAuthCard to an JSON string
  *
  * @return JSON string
  */
