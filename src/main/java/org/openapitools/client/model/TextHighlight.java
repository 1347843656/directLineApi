...
public class TextHighlight {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TextHighlight
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (TextHighlight.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in TextHighlight is not found in the empty JSON string", TextHighlight.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TextHighlight.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TextHighlight` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TextHighlight.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TextHighlight' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TextHighlight> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TextHighlight.class));

       return (TypeAdapter<T>) new TypeAdapter<TextHighlight>() {
           @Override
           public void write(JsonWriter out, TextHighlight value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TextHighlight read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of TextHighlight given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of TextHighlight
  * @throws IOException if the JSON string is invalid with respect to TextHighlight
  */
  public static TextHighlight fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TextHighlight.class);
  }

 /**
  * Convert an instance of TextHighlight to an JSON string
  *
  * @return JSON string
  */
