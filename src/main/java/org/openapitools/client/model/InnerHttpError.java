...
public class InnerHttpError {
  ...
  * @throws IOException if the JSON Object is invalid with respect to InnerHttpError
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (InnerHttpError.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in InnerHttpError is not found in the empty JSON string", InnerHttpError.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!InnerHttpError.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InnerHttpError` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!InnerHttpError.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'InnerHttpError' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<InnerHttpError> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(InnerHttpError.class));

       return (TypeAdapter<T>) new TypeAdapter<InnerHttpError>() {
           @Override
           public void write(JsonWriter out, InnerHttpError value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public InnerHttpError read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of InnerHttpError given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of InnerHttpError
  * @throws IOException if the JSON string is invalid with respect to InnerHttpError
  */
  public static InnerHttpError fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, InnerHttpError.class);
  }

 /**
  * Convert an instance of InnerHttpError to an JSON string
  *
  * @return JSON string
  */
