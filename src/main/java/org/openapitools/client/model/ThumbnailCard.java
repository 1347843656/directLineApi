...
public class ThumbnailCard {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ThumbnailCard
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (ThumbnailCard.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in ThumbnailCard is not found in the empty JSON string", ThumbnailCard.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ThumbnailCard.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ThumbnailCard` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if (jsonObj.get("images") != null && !jsonObj.get("images").isJsonNull()) {
        ...
        if (jsonArrayimages != null) {
          ...
          for (int i = 0; i < jsonArrayimages.size(); i++) {
            CardImage.validateJsonObject(jsonArrayimages.get(i).getAsJsonObject());
          };
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ThumbnailCard.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ThumbnailCard' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ThumbnailCard> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ThumbnailCard.class));

       return (TypeAdapter<T>) new TypeAdapter<ThumbnailCard>() {
           @Override
           public void write(JsonWriter out, ThumbnailCard value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ThumbnailCard read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ThumbnailCard given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ThumbnailCard
  * @throws IOException if the JSON string is invalid with respect to ThumbnailCard
  */
  public static ThumbnailCard fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ThumbnailCard.class);
  }

 /**
  * Convert an instance of ThumbnailCard to an JSON string
  *
  * @return JSON string
  */
