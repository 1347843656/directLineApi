...
public class ThumbnailUrl {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ThumbnailUrl
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (ThumbnailUrl.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in ThumbnailUrl is not found in the empty JSON string", ThumbnailUrl.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ThumbnailUrl.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ThumbnailUrl` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ThumbnailUrl.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ThumbnailUrl' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ThumbnailUrl> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ThumbnailUrl.class));

       return (TypeAdapter<T>) new TypeAdapter<ThumbnailUrl>() {
           @Override
           public void write(JsonWriter out, ThumbnailUrl value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ThumbnailUrl read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ThumbnailUrl given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ThumbnailUrl
  * @throws IOException if the JSON string is invalid with respect to ThumbnailUrl
  */
  public static ThumbnailUrl fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ThumbnailUrl.class);
  }

 /**
  * Convert an instance of ThumbnailUrl to an JSON string
  *
  * @return JSON string
  */
