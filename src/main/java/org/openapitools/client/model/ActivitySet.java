...
public class ActivitySet {
  ...
  * @throws IOException if the JSON Object is invalid with respect to ActivitySet
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (ActivitySet.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in ActivitySet is not found in the empty JSON string", ActivitySet.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!ActivitySet.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ActivitySet` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      if ((jsonObj.get("watermark") != null && !jsonObj.get("watermark").isJsonNull()) && !jsonObj.get("watermark").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `watermark` to be a primitive type in the JSON string but got `%s`", jsonObj.get("watermark").toString()));
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ActivitySet.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ActivitySet' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ActivitySet> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ActivitySet.class));

       return (TypeAdapter<T>) new TypeAdapter<ActivitySet>() {
           @Override
           public void write(JsonWriter out, ActivitySet value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ActivitySet read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ActivitySet given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ActivitySet
  * @throws IOException if the JSON string is invalid with respect to ActivitySet
  */
  public static ActivitySet fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ActivitySet.class);
  }

 /**
  * Convert an instance of ActivitySet to an JSON string
  *
  * @return JSON string
  */
