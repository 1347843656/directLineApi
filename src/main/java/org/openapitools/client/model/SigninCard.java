...
public class SigninCard {
  ...
  * @throws IOException if the JSON Object is invalid with respect to SigninCard
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (SigninCard.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in SigninCard is not found in the empty JSON string", SigninCard.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!SigninCard.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SigninCard` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SigninCard.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SigninCard' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SigninCard> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SigninCard.class));

       return (TypeAdapter<T>) new TypeAdapter<SigninCard>() {
           @Override
           public void write(JsonWriter out, SigninCard value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SigninCard read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of SigninCard given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of SigninCard
  * @throws IOException if the JSON string is invalid with respect to SigninCard
  */
  public static SigninCard fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SigninCard.class);
  }

 /**
  * Convert an instance of SigninCard to an JSON string
  *
  * @return JSON string
  */
