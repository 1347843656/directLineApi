...
public class HeroCard {
  ...
  * @throws IOException if the JSON Object is invalid with respect to HeroCard
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (HeroCard.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in HeroCard is not found in the empty JSON string", HeroCard.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!HeroCard.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HeroCard` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!HeroCard.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'HeroCard' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<HeroCard> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(HeroCard.class));

       return (TypeAdapter<T>) new TypeAdapter<HeroCard>() {
           @Override
           public void write(JsonWriter out, HeroCard value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public HeroCard read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of HeroCard given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of HeroCard
  * @throws IOException if the JSON string is invalid with respect to HeroCard
  */
  public static HeroCard fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, HeroCard.class);
  }

 /**
  * Convert an instance of HeroCard to an JSON string
  *
  * @return JSON string
  */
