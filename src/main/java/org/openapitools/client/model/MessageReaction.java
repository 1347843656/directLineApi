...
public class MessageReaction {
  ...
  public MessageReaction type(String type) {
  ...
  * @throws IOException if the JSON Object is invalid with respect to MessageReaction
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (MessageReaction.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in MessageReaction is not found in the empty JSON string", MessageReaction.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!MessageReaction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MessageReaction` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MessageReaction.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MessageReaction' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MessageReaction> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MessageReaction.class));

       return (TypeAdapter<T>) new TypeAdapter<MessageReaction>() {
           @Override
           public void write(JsonWriter out, MessageReaction value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MessageReaction read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of MessageReaction given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of MessageReaction
  * @throws IOException if the JSON string is invalid with respect to MessageReaction
  */
  public static MessageReaction fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MessageReaction.class);
  }

 /**
  * Convert an instance of MessageReaction to an JSON string
  *
  * @return JSON string
  */
