...
public class CardImage {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CardImage
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (CardImage.openapiRequiredFields.isEmpty()) {
          return;
        } else { // has required fields
          throw new IllegalArgumentException(String.format("The required field(s) %s in CardImage is not found in the empty JSON string", CardImage.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CardImage.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CardImage` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CardImage.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CardImage' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CardImage> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CardImage.class));

       return (TypeAdapter<T>) new TypeAdapter<CardImage>() {
           @Override
           public void write(JsonWriter out, CardImage value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CardImage read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of CardImage given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of CardImage
  * @throws IOException if the JSON string is invalid with respect to CardImage
  */
  public static CardImage fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CardImage.class);
  }

 /**
  * Convert an instance of CardImage to an JSON string
  *
  * @return JSON string
  */
